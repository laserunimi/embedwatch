...
switch( u8state ) {
  ...
  case 2:
    ...
    if (u8state == COM_IDLE) // COM_IDLE= desocupado o fin de recepcion de respuesta a telegrama
    {      
        u8state++;
        u32wait=millis();       
    }
  ...
  case 5:
   ...
   if (u8state == COM_IDLE){
    ...
    master_query(buffer, telegram[1] ); // send query (only once)
    u8state++; 
  }     
    break;
  ...
  case 7:
   ...
   if (u8state == COM_IDLE){
    ...
    master_query(buffer, telegram[3] ); // send query (only once)
    }
 ...
 case 8:
  ...
  case 9:
   ...
   if (u8state == COM_IDLE){
        ...
        master_query(buffer, telegram[4] ); // send query (only once)
        u32wait=millis(); 
    }
