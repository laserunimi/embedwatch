...
int main(int argc, char** argv) {
	...
	struct json_token_t tokens[500];
	memset(tokens, 0, sizeof(tokens));

    char my_json_string[8128];
    scanf("%s", my_json_string);

    json_size_t num_tokens =json_parse(my_json_string, strlen(my_json_string), tokens, sizeof(tokens) / sizeof(tokens[0]));
